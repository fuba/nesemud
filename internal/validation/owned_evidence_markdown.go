...
func ownedEvidenceNotes(r OwnedROMEvidence) string {
	...
	if r.Paused {
		if r.PauseReason != "" {
			return fmt.Sprintf("paused: %s", r.PauseReason)
		}
		if r.PausePC != 0 || r.PauseOpcode != 0 {
			return fmt.Sprintf("paused during run at PC=%04X opcode=%02X", r.PausePC, r.PauseOpcode)
		}
